...
/**
 * This class is used in order to create the graph example of hierarchy
 *
...
public class MainGraph {
	...
	public static void main(String[] args) {
		...
		try {
        	System.out.println("Launch Client Main Graph");
	...
	private static List<SiteItf> buildNodes(List<SiteItf> nodesList) throws RemoteException, MalformedURLException, NotBoundException {
		...
		System.out.println("Try to link every nodes for graph");
		//construct every nodes
		nodesList.get(1).ConstructSite(null, new SiteItf[]{nodesList.get(2), nodesList.get(5)});
		nodesList.get(2).ConstructSite(null, new SiteItf[]{nodesList.get(3), nodesList.get(4), nodesList.get(1)});
		nodesList.get(3).ConstructSite(null, new SiteItf[]{nodesList.get(2)});
		nodesList.get(4).ConstructSite(null, new SiteItf[]{nodesList.get(2)});
		nodesList.get(5).ConstructSite(null, new SiteItf[]{nodesList.get(6), nodesList.get(1)});
		nodesList.get(6).ConstructSite(null, new SiteItf[]{nodesList.get(5)});
